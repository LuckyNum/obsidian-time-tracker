<template>
    <div v-if="useTimerStore().initialized" class="activity-container">
        ...
        <div v-for="(entries,index) in listData" :key="entries[0]" class="activity-group">
            ...
            <div class="activity-content">
                <div v-for="entry in entries[1]" :key="entry.id" class="activity-item">
                    ...
                    <div class="activity-item-duration">
                        {{ formatMinuteDuration(moment.duration(entry.duration * 1000)) }}
                    </div>
                </div>
            </div>
        </div>
...
import {GroupBy, Query, SortOrder, TimeEntry} from "@/types.ts";
import {useTimerStore} from "@/store/TimerStore.ts";
import LoadingSpinner from "@/components/LoadingSpinner.vue";
import {moment} from "obsidian";
import {formatMinuteDuration, reverseMap} from "../lib/utils.ts";
import {COLORS} from "@/lib/constants.ts";

const props = defineProps<{
    query: Query
}>();
const listData = ref<Map<string, TimeEntry[] | undefined>>(new Map<string, TimeEntry[] | undefined>());

onMounted(async () => {
    if (useTimerStore().initialized) {
        listData.value = await getListData();
    }
})
...
async function getListData(): Promise<Map<string, TimeEntry[]>> {
    const timeRangeEntries = await useTimerStore().getTimeRangeEntries(props.query);
    if (props.query.groupBy && props.query.groupBy === GroupBy.TAG) {
        // 标签分组
        return Array.from(timeRangeEntries.values()).flat().reduce((acc, current) => {
            // 查找已经合并的元素，如果存在则更新duration和count
            const existing = acc.find(item => item.title === current?.title && item.tag === current.tag);
            if (existing) {
                existing.duration += current?.duration || 0;
        ...
        // 默认时间分组
        if (props.query.sort) {
            if (props.query.sort === SortOrder.ASC) {
                return reverseMap<string, TimeEntry[]>(timeRangeEntries);
            }
        }
